...
- You report; the author (or `implementer`) fixes.
...
1. **Design** — right surface placement per `AGENTS.md`? Smallest viable shape? Does it duplicate `@maeton/*` functionality (constitution violation)? Sound failure modes?
...
3. **Security** — input validated at every trust boundary (Zod)? AuthZ/tenancy enforced? No secret leakage, injection, or unsafe deserialization? Schema-boundary respected?
...
Check and report any violation explicitly:

- `any` / unjustified `@ts-ignore` / `@ts-expect-error`
- missing Zod at a trust boundary
- cross-schema foreign key (`public` ↔ `admin`)
- irreversible migration without justification
- module re-implementing `@maeton/*`-owned infrastructure
- env-var module activation instead of `maeton.config.ts`
...
- No style noise — Prettier/ESLint own formatting. Don't report what the formatter fixes.
