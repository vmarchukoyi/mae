...
# Project Explore — one-time survey + incremental refresh
...
`CONSTITUTION.md` (engineering hard rules) and `docs/PROJECT.md` (business context)
...
| `default_route` | `auto` | `/feature-start` size→route mapping: `auto` (XS/S→quick, M→standard, L/XL→full) or a pinned route |
...
**Brownfield:**
- Read `CONSTITUTION.md`, `AGENTS.md`, existing `docs/` — the stated intent.
...
- Run the level-adaptive foundation session: stack, first surfaces, first modules,
  data boundaries — anchored to `CONSTITUTION.md`'s stack lock (do not re-litigate
...
Copy `docs/_templates/project.md` and fill it. This is the business brief an agent
reads to keep the goal in view:
...
For the **Maeton template itself**, the "product" is the reusable SaaS foundation —
its users are OTAKOYI engineers deriving projects. For a **derived project**, it is
that client's application.

## Step 4 — write `docs/architecture-map.md` (structural map)

Copy `docs/_templates/architecture-map.md`. Two rules make this document trustworthy:
...
For each foundational choice made in Step 2, write a short decision record (dispatch
`architect`, or inline if trivial) and a starter task list. Skip entirely for
brownfield — you are describing, not deciding.
...
- Two documents, never one. `docs/PROJECT.md` (business) ≠ `CONSTITUTION.md`
...
- Read-heavy work can go to `codebase-explorer` (Haiku); keep the main context clean.
