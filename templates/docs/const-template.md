# Constitution — engineering law · <PROJECT NAME>

> **This is a scaffolded template.** It lives at `docs/constitution.md` in your project.
> `/mae:init` fills every `<…>` from the constitution interview. This file is engineering
> **law** — the non-negotiables the mae workflow enforces. Business context (idea, goals,
> roles, scenarios) lives in a **separate** `docs/PROJECT.md`; never merge the two.
> Detailed, path-scoped rules live under `.claude/rules/`. Changes to the constitution
> require senior review and a standalone PR — they cannot ride inside a feature change.
...
<The short, absolute list the workflow checks on every change. Keep it to what you truly
never allow. Examples — edit to your stack:>

- No untyped escape hatches (e.g. `any`), no type-checker suppression without a justified inline comment.
