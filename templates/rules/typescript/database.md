...
Loaded when editing Prisma schema, migrations, or `@your-org/db`. Authoritative source:
`docs/constitution.md` § "Hard rules" + § "Stack lock".
...
- **Tenancy:** tenant-scoped tables carry `org_id`; reads and writes go through the tenancy mechanism (`@your-org/multitenancy`). Call out any deliberately unscoped access.
...
- **`@your-org/db` owns the client and schema.** A module does not spin up its own Prisma client or duplicate models it can import.
