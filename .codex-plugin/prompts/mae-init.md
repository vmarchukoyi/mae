...
Run this **once** to stand a project up on the mae workflow. It does two jobs in one pass,
in order:
...
Run it as an **interview**, not a batch job: **one question at a time**, each with your
recommended answer and a one-line why; research facts from the repo instead of asking them;
no scaffolding before the open decisions are resolved. Deterministic file placement is
delegated to the scaffolder — your job is to interview and to fill placeholders.

The scaffolder is invoked as (point it at wherever you cloned the mae repo):
`node <path-to-mae>/scripts/scaffold.mjs --target . [--e2e] [--ci]`

**Re-run to refresh, not to redo.** On a re-run the scaffolder skips existing files and the
survey does an *incremental* re-survey of only the changed modules (Step 9). init is
...
- **Map exists** → this is a **re-run**. The scaffold is already in place; do a light drift
  check (Step 8) and jump to the **incremental refresh** in Step 9 — read the map's
...
Ask **one question**: how much init decides on its own vs asks. This governs both halves
(scaffold and survey).
...
**Per-developer config — `.claude/sdd.local.md`.** Local-only, gitignored, auto-created on
first run with self-documenting defaults; degrade gracefully if it is absent or unwritable
(fall back to the defaults, don't block). Keys:
...
When absent, create it with these defaults and tell the user it was seeded.
...
Otherwise → **existing-project (brownfield)**. If it is genuinely ambiguous, ask which the
user intends. This axis decides both the scaffold branch (Step 4/5) and the survey mode
(Step 8).
...
2. If yes, offer two paths:
   - **Preset frameworks** — a curated shortlist, scaffolded with the framework's
     **official generator**, never a bundled copy:
...
1. **Analyze the codebase yourself:** report stack, layout, test/build commands, and
   existing docs — read the manifests, config files, and entry points as evidence.
...
Hold the answers; you fill the `docs/constitution.md` placeholders after scaffolding (Step
7 writes the template with `<…>` placeholders).
...
testing, git) plus the rest of the SDD layer. These are stack-agnostic; any stack-specific
rules are the project's to add later. Run:

`node <path-to-mae>/scripts/scaffold.mjs --target .`

Ask (recommend yes, opt-out) two more questions, adding flags accordingly:
- **E2E testing?** → add `--e2e` (scaffolds the e2e agent pair and registers the Playwright
...
- Inventory surfaces: `apps/*`, `packages/*`, `modules/*`. For each, read the entry file,
  `package.json`, and any existing surface doc.
- Detect the machine facts from evidence, never from guesswork (Step 9 rule).

**Greenfield:**
- Run the level-adaptive foundation session: stack, first surfaces, first modules, data
  boundaries — anchored to `docs/constitution.md`'s stack lock (do not re-litigate locked
  choices; choose only what the constitution leaves open).
...
**`docs/PROJECT.md` (business context).** Copy `project.md` from the scaffolded
`docs/_templates/` and fill it:
...
For a **derived project**, the "product" is that project's application — its users are the
engineers and stakeholders building on top of it.
...
2. **Machine-readable frontmatter, evidence-only.** Fill `test_cmd`, `lint_cmd`,
   `typecheck_cmd`, `build_cmd`, `migration_tool`, `frontend` from **proof in the repo** (a
   script in `package.json`, a config file, a lockfile). **No proof → leave the value an
   empty string. Never guess.** Every later stage reads its commands from here, so a wrong
   guess poisons the whole pipeline; a blank is safe.

Body: one row per surface (path, kind, role, depends-on, consumed-by), the boundaries that
bite (schema separation, tenancy, package ownership), and a coarse mermaid of how surfaces
connect. Link out to the per-surface docs under `docs/projects/` and `docs/packages/` — do
not duplicate them here.

## Step 10 — Greenfield only: record the foundation

For each foundational choice made in Step 8, write a short decision record (inline is fine
unless it changes a shared boundary, in which case open it as its own planning
...
- **Scaffold drift.** The scaffolder skips existing files. For a skipped-but-outdated file
  (its `mae-scaffold-version` marker older than the installed version), show a per-file diff
...
- **Map refresh.** Read the map's `reflects_commit`, then run `git rev-parse HEAD` and
  ...
  - Diff touches **≤ half** the modules → **incremental refresh**: re-scan only the changed
    surfaces, update just their sections, re-stamp `reflects_commit`.
...
## Run next
Start a feature once the map is approved:  /mae-start
```

## Rules

- One command, one time. init both bootstraps and surveys; re-run it only to refresh, never
