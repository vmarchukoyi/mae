...
Run from the repo root, stopping nothing early — collect all results. Use the project's configured commands (from `package.json`, `docs/constitution.md`, or the conventions doc). The typical shape:
...
For a scoped change you may add the package-filtered form (e.g. `pnpm --filter <package> test`) but always confirm the repo-level gate too.
...
- One-line read on what it points to (which file/area), so the author can act. You diagnose direction; you do not fix.
